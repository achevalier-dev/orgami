...
publish_front_page() {
  ...
  page=$(mktemp)
  {
    ...
    echo
    if [[ -f $DIR/map/EXECUTIVE.md ]]; then
      echo "Not an engineer? [**The same week in plain language**]($path/EXECUTIVE.md) —"
      echo "no jargon, and it says when something is waiting on you."
      echo
    fi
    ...
    echo "| | |"
    echo "|---|---|"
    [[ -f $DIR/map/EXECUTIVE.md ]] &&
      echo "| [Plain language]($path/EXECUTIVE.md) | every week without the jargon, and what needs a decision |"
...
cmd_publish() {
  ...
  if compgen -G "$DIR/reports/exec/*.md" >/dev/null; then
    mkdir -p "$dest/reports/exec"
    cp -f "$DIR/reports/exec/"*.md "$dest/reports/exec/" 2>/dev/null || true
  fi
  local f
